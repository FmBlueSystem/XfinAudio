# Energy Arc Sequencing Specification

## Purpose

Define fragmentation-resistant arc subset selection for the four energy-arc playlist strategies
(`harmonic_journey`, `warmup`, `build`, `peak_time`): each sequence is built by selecting a feasible
subset of the full BPM-reachable pool instead of permuting a pre-truncated shortlist, so the result
no longer depends on whether the pool arrived desktop-capped or raw.
