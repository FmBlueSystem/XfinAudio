# Loudness Analysis Specification

## Purpose

Define XfinAudio's EBU R128 loudness measurement capability: per-track integrated loudness (LUFS),
loudness range (LRA), and true peak (dBTP) measured with a pinned FFmpeg executable and persisted as
a versioned profile; a user-selectable LUFS target band enforced as a playlist pool filter; and
always-on tag write-back, the documented exception to XfinAudio's read-only scanning contract.
