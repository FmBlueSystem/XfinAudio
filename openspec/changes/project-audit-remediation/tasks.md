...
- [x] **13.1 Native 4R corrections** — RED: prove call-time export delegation, translator installation during package smoke, and missing-last descending sorting. GREEN: inject export dependencies in the owning coordinator without extracted-module back-imports, remove callable wrappers/redundant Qt import, install translations before smoke exit, and partition missing numeric rows after directional sorting. VERIFY: 61 focused tests and all ordered gates.
- [x] **14.1 Final native corrections** — RED: prove metadata-status export honors replacement Serato discovery. GREEN: route both metadata worklist variants through the injected bundle. REFACTOR: replace duplicated dynamic fallback classes with the canonical `ExportDependencies` bundle/resolver while retaining standalone defaults and no reverse imports. VERIFY: 28 focused tests and all ordered gates.
