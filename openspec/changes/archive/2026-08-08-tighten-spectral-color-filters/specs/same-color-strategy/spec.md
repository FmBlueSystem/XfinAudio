...
`same_color` MUST NOT acquire an energy filter. Energy MUST remain a weighted scoring preference, exactly as before this change; only the colour constraint tightens.
...
(Previously: When no track matched the anchor colour, `_apply_color_filter` returned the entire unfiltered pool and emitted a "falling back to unfiltered scoring" warning; the existing durable spec and the `tighten-same-color-energy` characterization tests encode that fallback.)
