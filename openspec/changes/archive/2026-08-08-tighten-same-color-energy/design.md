...
RED/GREEN/BLUE use label equality plus exact energy only; zero centroid/rolloff remains eligible. MIXED alone uses named, calibration-provisional constants: `COLOR_RGB_L1_MAX` (initial value `0.08`), `COLOR_CENTROID_REL_MAX` (initial `0.15`), and `COLOR_ROLLOFF_REL_MAX` (initial `0.15`) — renamed from the original `MIXED_`-prefixed names by the successor change `tighten-spectral-color-filters`, which extends the same gate to every dominant-color label; the predecessor applied them to MIXED only. The rule SHAPE — label equality plus a bounded anchor-relative RGB L1 / centroid / rolloff proximity gate — is normative; the literal numeric values are PROVISIONAL pending listening calibration and MUST be expressed as named constants so a post-calibration change touches only the constant definitions, not the rule. MIXED profiles require finite ratios with positive sums and finite positive centroid/rolloff. Relative delta is `abs(candidate-anchor)/anchor`; zero/invalid denominators fail closed.
