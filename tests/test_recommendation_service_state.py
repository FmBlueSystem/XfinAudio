...
def _dispatch_service(strategy_data: str) -> tuple[RecommendationService, dict[str, Any]]:
    service = RecommendationService(cast(Any, object()))
    combo = _StrategyCombo([("Display", strategy_data)])
    build_screen = SimpleNamespace(
        recommend_button=_Button(),
        strategy_combo=combo,
        spectral_cohesion_value=lambda: 50,
    ...
    calls: dict[str, Any] = {"records_route": 0, "context_route": 0, "started": []}

    def _records_route(_controls: Any, _strategy: str | None = None) -> list[Any]:
