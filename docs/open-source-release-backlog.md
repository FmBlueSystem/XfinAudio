...
| P1 | Legal review for binary redistribution | Pending: counsel or qualified legal review must assess GPLv3, PySide6/Qt, mutagen, and dependency redistribution obligations before binary/app bundle publication. No legal clearance is implied. |
| P1 | Clean app-bundle launch validation | Pending: follow `docs/clean-macos-account-validation.md`; generated temp app bundle must launch on a clean macOS test account with PyInstaller warnings resolved or explicitly triaged. |
| P1 | Developer ID signing plan | Pending: follow `docs/macos-signing-notarization-dmg-plan.md`; generated `.app` must be signed only after clean-account and warning gates pass. |
| P1 | Notarization plan | Pending: follow `docs/macos-signing-notarization-dmg-plan.md`; notarization and stapling must be verified before binary redistribution claims. |
| P1 | DMG distribution plan | Pending: follow `docs/macos-signing-notarization-dmg-plan.md`; DMG layout and mount/copy/launch checks remain unexecuted. |
